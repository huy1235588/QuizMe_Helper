...
def parse_chemistry_questions(file_path):
    ...
    Xử lý format: A, B cùng dòng; C, D cùng dòng
    """
    questions = []
    ...
    question_blocks = re.split(r'Câu \d+:', content)[1:]  # Bỏ qua phần tử trống đầu tiên
    ...
    for i, block in enumerate(question_blocks, 1):
        ...
        for line in lines[1:]:
            ...
            # Xử lý dòng có thể chứa nhiều lựa chọn (A, B hoặc C, D)
            # Tìm tất cả pattern A., B., C., D. trong dòng
            option_matches = re.finditer(r'([A-D])\.\s*([^A-D]*?)(?=[A-D]\.|$)', line)
        ...
        # Nếu không tìm được options với regex trên, thử phương pháp cũ
        if len(options) == 0:
            for line in lines[1:]:
                line = line.strip()
        ...
        if len(options) == 4:  # Chỉ bao gồm câu hỏi có 4 lựa chọn
            questions.append({
                'question_number': i,
...
def create_single_quiz(all_questions):
    ...
    # Lưu file
    output_file = f"chemistry_quiz_{quiz_id}.json"
...
def main():
    ...
    input_file = r"e:\Downloads\ha\hoá học.txt"
